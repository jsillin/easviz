...
extradoms = True
centerlat = [45,30,42.5,42,36]
centerlon = [110,90,76.5,88,86]
state_abbr = ['mt','la','ny','il','tn']
...
for i in range(6,49):
    fhr = str(i)
    #outfname = ''
    #if path.isfile(outfname)==True:
    #    print('Forecast Hour '+str(i)+' Completed Already')
    #else:
    ...
    #fig.canvas.draw()
    fig.tight_layout()
    plt.savefig('EC_fivepanel_summer_'+fhr+'.png',bbox_inches='tight',pad_inches=0.1)
    if extradoms == True:
        for j in range(len(centerlon)):
            ...
            #fig.canvas.draw()
            fig.tight_layout()
            plt.savefig(state_abbr[j]+'_fivepanel_summer_'+fhr+'.png',bbox_inches='tight',pad_inches=0.1)
    ...
    plt.savefig('NE_fivepanel_summer_'+fhr+'.png',bbox_inches='tight',pad_inches=0.1)
    plt.clf()
    ...
    if plot_overview == True:
        ...
        #fig.canvas.draw()
        fig11.tight_layout()
        plt.savefig('EC_overview_summer_'+fhr+'_.png',bbox_inches='tight',pad_inches=0.1)
        ...
        plt.savefig('NE_overview_summer_v1_'+dtfs+'.png',bbox_inches='tight',pad_inches=0.1)
        plt.clf()
